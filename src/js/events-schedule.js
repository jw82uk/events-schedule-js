...
function eventsSchedule(userSettings) {
    ...
    let locale = settings.time_locales;
    let timezone = settings.time_zone;

    // Insert time zone, if one has been specified, into date and time objects
    if (timezone) {
    ...
    function createEventsSchedule() {
        ...
        // let todayDate = new Date('Sat Oct 24 2020 12:32:00 GMT+0100 (British Summer Time)'); // FOR TESTING PURPOSES ONLY
        const todayDateUTC = todayDate.toUTCString();
        const currrentTime = new Date(todayDateUTC).getTime();

        // Determine the status (past, present, future) of each event in eventsObj. 
        // Add an appropriate event status key/value pair to the event, and update counters.
        for (let i = 0; i < eventsObj.events.length; i++) {

            let eventStartTime = new Date(eventsObj.events[i].event_start).getTime();
            let eventEndTime = new Date(eventsObj.events[i].event_end).getTime();
            let startingSoonMs = (settings.starting_soon_mins * 60) * 1000;
            ...
            eventsObj.events[i].start_time_ms = eventStartTime;

            // Add a 'duration_ms' key/value (for chronological ordering)
            eventsObj.events[i].duration_ms = eventEndTime - eventStartTime;

            // Is the event starting soon?
            if (startingSoonMs) {
        ...
        for (let i = 0; i < eventsObj.events.length; i++) {
            eventsObj.events[i].relevant = false;
        }
        ...
        if (settings.only_events_tagged.length) {
            ...
            settings.only_events_tagged.forEach(element => {
                ...
                for (let i = 0; i < eventsObj.events.length; i++) {
                    ...
                    let indexOfTag = eventsObj.events[i].tags.indexOf(element);

                    // If the event is NOT already set to relevant (it may have been if there are multiple tags to check)
                    // AND it does contain the tag we're checking for
                    if (!eventsObj.events[i].relevant && indexOfTag > -1) {
                        // Set relevance to true
                        eventsObj.events[i].relevant = true;
                    }
                }
            });
        }
        ...
        urlParamValue = getAllUrlParams()[settings.query_string_key];

        // If a query_string_key has been specified, and it has a value
        if (settings.query_string_key && urlParamValue) {
            ...
            for (let i = 0; i < eventsObj.events.length; i++) {
                ...
                let indexOfUrlParam = eventsObj.events[i].tags.indexOf(urlParamValue);

                // If the event is already marked as relevant (from check 1 above)
                // AND does NOT contain the relevant tag (i.e. index position of -1)
                if (eventsObj.events[i].relevant && indexOfUrlParam < 0) {
        ...
        const maxEvents = settings.max_events;

        // Reset relevant events count
        relevantEventsCount = 0;

        // Events container
        allEvents = '';

        // If necessary, order events chronologically
        ...
        if (presentEventsCount || futureEventsCount) {
            ...
            for (let i = 0; i < eventsObj.events.length; i++) {
                ...
                if (settings.show_live_events) {
                    if ((eventsObj.events[i].event_status == 'present') && (eventsObj.events[i].relevant == true) && (relevantEventsCount < maxEvents)) {

                        allEvents += buildEvent(i);
                ...
                if (settings.show_upcoming_events) {
                    if ((eventsObj.events[i].event_status == 'future') && (eventsObj.events[i].relevant == true) && (relevantEventsCount < maxEvents)) {
        ...
        if (relevantEventsCount > 0) {
            ...
            const eventsWrapperElement = settings.events_wrapper_html_element;

            // If the number of results should display
            ...
            // User defined callback function to run after events load
        ...
        setTimeout(function () {
            ...
            const eventSelector = '#' + settings.container_id + ' .events-schedule__event';

            let allEvents = document.querySelectorAll(eventSelector);

            for (let i = 0; i < allEvents.length; i++) {
                allEvents[i].classList.add('events-schedule__event--opaque');
            }
        }, 50);
    }
    ...
    function buildEvent(eventPos) {
        ...
        let eventStart = new Date(eventsObj.events[eventPos].event_start);
        let eventEnd = new Date(eventsObj.events[eventPos].event_end);
        let eventHolder = '';
        let eventElement = settings.events_html_element;
        let headingLevel = settings.events_heading_level;

        eventHolder += '<' + eventElement + ' class="events-schedule__event events-schedule__event--' + eventsObj.events[eventPos].event_status + ' events-schedule__event--starting-soon-' + eventsObj.events[eventPos].starting_soon;
        ...
        // If images should show AND the event has an image
        ...
        // Begin text wrapper
        eventHolder += '<div class="events-schedule__details">';

        // Add title
        eventHolder += '<' + headingLevel + ' class="events-schedule__title">' + eventsObj.events[eventPos].event_title + '</' + headingLevel + '>';

        // Date, time, and status (live or upcoming)
        eventHolder += '<p class="events-schedule__time">';
        ...
        if (settings.show_tags_on_events && eventsObj.events[eventPos].tags.length) {
            ...
            let eventTags = eventsObj.events[eventPos].tags;
    ...
    function updateSelectMenu() {
        ...
        // If the url parameter key has a value
        if (urlParamValue) {
    ...
    const eventsFilter = document.getElementById(settings.filter_id);

    // If a filter_id and query_string_key are specified
